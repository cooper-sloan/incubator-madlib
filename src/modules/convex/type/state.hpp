...
class MLPBatchState {
    template <class OtherHandle>
    friend class MLPBatchState;

public:
    MLPBatchState(const AnyType &inArray) : mStorage(inArray.getAs<Handle>()) {
        rebind();
    }
     ...
     */
    template <class OtherHandle>
    MLPBatchState &operator=(const MLPBatchState<OtherHandle> &inOtherState) {
        for (size_t i = 0; i < mStorage.size(); i++) {
            mStorage[i] = inOtherState.mStorage[i];
        }
